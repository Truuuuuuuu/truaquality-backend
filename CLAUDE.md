...
- Tests: `npm test`
  ...
  - **Variables exported in your shell OVERRIDE `test.env`.** `src/testing/globalSetup.ts` refuses the whole
    ...
    override the host in the URL, so "the hostname says localhost" is not enough. It also refuses the run
    outright if `SUPABASE_SECRET_KEY`, `MQTT_URL`, `MQTT_USERNAME` or `MQTT_PASSWORD` is set — `test.env`
    omits them deliberately, and a shell export would otherwise put the real ones back.
